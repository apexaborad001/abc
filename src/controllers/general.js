...
let generalQueryController = {
	emailAndPhone: async (req, res) => {
		...
		try {
			let responseObject = [];
			//let conversationData = req.body.conversationData;
			if (conversationData.previousIntentName === undefined) conversationData.previousIntentName = "agent.specific.contactUs";
			...
			if (conversationData.previousIntentName === "agent.specific.shareSolution" && conversationData.readOnlineAskedOnIndustries) {
			...
			if (conversationData.previousIntentName === "agent.specific.shareSolution" && conversationData.frameworkServicesForReadOnline) {
				if (conversationData.readSelector === "Online" && emailData.isGiven) {
					if (emailData.verifiedStatus && emailData.isBusinessEmail) {
			...
			if (conversationData.previousIntentName === "agent.specific.shareSolution" && conversationData.technologyExcellenceForReadOnline) {
				if (conversationData.readSelector === "Online" && emailData.isGiven) {
					if (emailData.verifiedStatus && emailData.isBusinessEmail) {
			...
			if (conversationData.previousIntentName === "agent.specific.shareSolution" && conversationData.innovationServicesForReadOnline) {
				if (conversationData.readSelector === "Online" && emailData.isGiven) {
					if (emailData.verifiedStatus && emailData.isBusinessEmail) {
			...
			if (conversationData.previousIntentName === "agent.specific.shareSolution" && conversationData.captureEmailForShareSolution) {
			...
			if (conversationData.previousIntentName === "agent.specific.showSolutions") {
				if (emailData.isGiven) {
					if (emailData.verifiedStatus && emailData.isBusinessEmail) {
						let solutionname = conversationData.solutionName;
						conversationData.isEmailVerified = true;
